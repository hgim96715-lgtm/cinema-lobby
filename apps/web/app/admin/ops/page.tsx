...
type ResultRow = { id: string; ok: boolean };
type ResultState =
...
export default function AdminOpsPage() {
  ...
  async function seedOne(machineId: string) {
    ...
    setResult(null);
    try {
      await seedPoolRequest(token, machineId, pages);
      setResult({ kind: 'rows', rows: [{ id: machineId, ok: true }] });
    } catch (error) {
      setResult({
        kind: 'error',
        message: error instanceof Error ? error.message : String(error),
    ...
    <main className="admin-main">
      ...
      <div className="admin-ops-grid">
        ...
        <p className="admin-ops-desc">
          TMDB에서 영화를 가져와 풀을 채웁니다. cron은 매일 KST 02:00 자동 실행.
        </p>
